...
def convert_avi_to_mp4(input_file, output_file):
    try:
        (
            ffmpeg
            .input(input_file)
            .output(output_file, vcodec='libx264', acodec='aac')  # Specify video and audio codecs
            .run(overwrite_output=True)  # Use overwrite_output=True to overwrite if output exists
        )
